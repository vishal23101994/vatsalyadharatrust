...
export async function generateReceiptPDF(donation: any): Promise<string> {
  ...
  const stampX = doc.page.width - stampWidth - bottomMargin;
  const stampY = pageHeight - 230;

  const signX = doc.page.width - signWidth - bottomMargin + 10;
  const signY = pageHeight - 160;

  // Stamp (slightly transparent looks more authentic)
